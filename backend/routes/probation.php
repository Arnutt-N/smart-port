    ...
    try {
        $summaryStmt = $pdo->query("
            SELECT
                ...
                SUM(CASE WHEN DATEDIFF(end_date, CURDATE()) BETWEEN 1 AND " . PROBATION_NEAR_THRESHOLD_DAYS . " THEN 1 ELSE 0 END) AS near_deadline,
        ...
        foreach ($rows as $r) {
            ...
            if ($rem > 0) $inProgress++;
            if ($rem >= 1 && $rem <= PROBATION_NEAR_THRESHOLD_DAYS) $nearDeadline++;
            if ($rem < 0) $overdue++;
        }
    }
