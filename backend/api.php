    ...
    case 'dashboard':
        if ($method == 'GET') {
            ...
            try {
                ...
                $stmt = $pdo->query("SELECT COUNT(*) as c FROM vw_probation_dashboard WHERE remaining_days BETWEEN 1 AND " . PROBATION_NEAR_THRESHOLD_DAYS);
                ...
                try {
                    ...
                    $stmt = $pdo->query("
                        SELECT COUNT(*) as c FROM probation_enrollment
                        WHERE overall_status = 'IN_PROGRESS'
                        AND DATEDIFF(end_date, CURDATE()) BETWEEN 1 AND " . PROBATION_NEAR_THRESHOLD_DAYS . "
                    ");
                    $probationNear = (int) $stmt->fetch(PDO::FETCH_ASSOC)['c'];
