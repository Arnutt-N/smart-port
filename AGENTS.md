...
**Single source of truth for all coding agents** (Cursor, Claude Code, Codex, and others).

`CLAUDE.md` exists only as a pointer for Claude Code — read this file instead.
...
- **Deploy without Actions:** enable **Auto-Deploy** on Render for `main`, or `.\scripts\deploy-render.ps1` with `RENDER_DEPLOY_HOOK_URL` in env/`.env` (URL never printed). After triggering, the script polls the live header gate (`verify-live-headers.mjs`) every 30s until PASS or timeout (default 15min, `-VerifyTimeoutMin`; `-SkipVerify` to skip).
