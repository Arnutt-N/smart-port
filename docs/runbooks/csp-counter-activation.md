...
| `storage=unavailable` | เป็นได้ 3 อย่าง: ยังไม่ได้ทำข้อ 1 (ไม่มีตาราง) · backend ต่อฐานข้อมูลไม่ได้ · query ล้มเหลวกลางคัน | ถ้ายังไม่ได้ทำข้อ 1 ให้รัน SQL ก่อน · ถ้าทำแล้วให้ดู log ของ `smartport-backend` หา `[csp-report] summary` |
| `ครอบไม่ครบ` / `ตัวนับเริ่มนับ ...` | ตัวนับเริ่มเก็บข้อมูลหลังต้นหน้าต่างที่ถาม (เช่นเพิ่งรัน SQL เมื่อวานแล้วถาม 7 วัน) | รอจนตัวนับเก็บครบตามจำนวนวันที่ถาม หรือถามด้วย `--days` ที่สั้นลง — **ห้ามตีความว่าสะอาด** |
| `backend ตอบหน้าต่าง N วัน ไม่ใช่ M ที่ขอ` | มีอะไรระหว่างทางตัดพารามิเตอร์ทิ้ง คำตอบจึงมาจากช่วงเวลาที่ไม่ได้ถาม | แจ้งทีมพัฒนา — ห้ามใช้ผลนี้ตัดสิน |
...
1. **ตัวนับนี้เป็นหลักฐานเสริม ไม่ใช่ตัวแทนของ Render log สำหรับรอบแรก** เพราะข้อมูลเริ่มนับ
   ...
   **ข้อนี้เครื่องมือบังคับให้เองแล้ว ไม่ต้องพึ่งความจำ**: ระบบส่งค่า "เริ่มนับเมื่อไร" มาด้วย และ
   สคริปต์จะตอบว่าไม่ผ่านทันทีถ้าตัวนับยังครอบไม่ถึงต้นหน้าต่างที่ถาม
